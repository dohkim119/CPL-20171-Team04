...
{
	m_hComm = CreateFile(L"//./" + portname, GENERIC_READ | GENERIC_WRITE, 0, 0, OPEN_EXISTING, 0, 0); //시리얼 포트 오픈
	if (m_hComm == INVALID_HANDLE_VALUE)  //포트 정상적으로 열렸는지확인
	{
		return false;  //비정상 경우 false 반환
	}
	else
		return true;   //정상의 경우 true 반환
}
...
bool CSerialPort::SetCommunicationTimeouts(DWORD ReadIntervalTimeout,DWORD ReadTotalTimeoutMultiplier, DWORD ReadTotalTimeoutConstant,DWORD WriteTotalTimeoutMultiplier, DWORD WriteTotalTimeoutConstant) //통신 포트에 관한 Timeout 설정
{
